...
for i in range(1, 24):
    ...
    for row in tuple(ws.rows):
        ...
        if (broad_category is not None) and (broad_category != ""):
            ...
            broad_category_data.append([title, abstract, broad_category])

        if (health_code is not None) and (health_code != ""):
            # print title + " ::: " + health_code
            health_codes_data.append([title, abstract, health_code])

        if (research_activity_code is not None) and (research_activity_code != ""):
            # print title + " ::: " + research_activity_code
            research_activity_code_data.append([title, abstract, research_activity_code])
...
# with open ('research_activity_code_data.obj', 'rb') as fp:
#     research_activity_code_data = pickle.load(fp)
